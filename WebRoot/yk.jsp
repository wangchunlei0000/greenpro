...
@keyframes myfirst
{
  ...
  <body>
   ...
   <div style="height:5%;width:100%"><jsp:include page="bbanner.jsp"></jsp:include></div>
	
  <div style="width:100%">
 		<center><br/><br/>
             <h2> 融资项目</h2><br>
    <form action="y_yfindByTitle.action" method="post">
    <table width="505" height="150">
		...
		<input height="40px" type="radio" name="r2.id" value="${ty.id}">${ty.style}&nbsp; &nbsp;   &nbsp; 
		</s:iterator>
        </td></tr>
    	<tr><td><div class="ani"></div>&nbsp;<font size="4">项目进程</font></td>
    	<td height="40px">
    	<input type="radio" name="r1.id" value="2">融资中 &nbsp; &nbsp; 
    	<input type="radio" name="r1.id" value="4">融资结束
    	</td>
    	</tr> 
    	<tr><td style="text-align: center"colspan="2"><input class="zhubu" type="submit" value="筛选"/>&nbsp;&nbsp;
    	 <a class="zhubu" href="y_yfindByTitle.action">查看全部</a></td></tr>
    </table>
    </form><hr/>
    ...
    <div style="padding-left:10px;padding-right:0px;margin:25px;float:left;width:65%;height:35%;height:50%">
       ...
       <div style="padding-left:20px;padding-right:30px;margin:25px;float:left;width:550px;">
    	<div style="height:80%">
    			...
    			<a href="cypoj_uusers?po.pojid=${lpp.pojid}">${lpp.name}</a>&nbsp;&nbsp;
    			<a href="y_yjubao.action?p.pojid=${lpp.pojid}" onclick="return confirm('确定举报这个项目?');">举报</a>&nbsp;&nbsp;
    			<s:if test="(#session.role)==null"></s:if>
    			<s:else>
    				<s:if test="(#lpp.rongziState.id)==2">
    						<a href="rongzipoj_money.action?p.pojid=${lpp.pojid}">参加融资</a>
    				</s:if>
    			</s:else></div>
    	</div>
    </s:iterator>
    ...
    <div  style="width:100%;float:left;">
     <form method="post" action="rongzipoj_test1.action?">
	    	<s:if test="pp.currentPage==1">上一页</s:if>
	    	<s:else><a href="rongzipoj_test1.action?pp.currentPage=${pp.currentPage-1}">上一页</a></s:else>
	    	&nbsp;&nbsp;
	    	<s:if test="pp.currentPage==pp.pageCount">下一页</s:if>
	    	<s:else><a href="rongzipoj_test1.action?pp.currentPage=${pp.currentPage+1}">下一页</a></s:else>
	    	&nbsp;&nbsp;
	    	共${pp.pageCount}页&nbsp;第&nbsp;
