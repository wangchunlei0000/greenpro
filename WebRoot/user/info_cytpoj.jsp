 ...
 .zhubu{
  ...
  <center><br/><br/>
   <%  
		...
		if(request.getAttribute("c.touziPoj.money")!=null&!"".equals(request.getAttribute("c.touziPoj.money"))){
			ii=request.getAttribute("c.touziPoj.money").toString();
		...
		if(request.getAttribute("c.touziPoj.nowmoney")!=null&!"".equals(request.getAttribute("c.touziPoj.nowmoney"))){
			iii=request.getAttribute("c.touziPoj.nowmoney").toString();
		    ddd=Double.parseDouble(iii);  
		    biggg = new java.math.BigDecimal(df.format(ddd));//out.println(biggg);
		}
	%>
  
  <h3>${c.touziPoj.name}</h3>
     <table class="yy" width="433" height="395">
    	<tr><td width="43%">项目号:</td><td>${c.touziPoj.pojid}</td></tr>
    	<tr><td>项目名:</td><td>${c.touziPoj.name}</td></tr>
    	<tr><td>发起人:</td><td>${c.users.userid}</td></tr>
    	<tr><td>参与金额:</td><td><%=big %></td></tr>
    	<tr><td>投资状态:</td><td>${c.touziPoj.touziState.style}</td></tr>
    	<tr><td>目标资金:</td><td><%=bigg %></td></tr>
    	<tr><td>现投资金额:</td><td><%=biggg %></td></tr>
    	<tr><td>所属区域:</td><td>${c.touziPoj.area}</td></tr>
    	<tr><td>开始时间</td><td>${c.touziPoj.starttime}</td></tr>
    	<tr><td>项目图片:</td><td><img src="${c.touziPoj.icon}" width="140" height="109"></td></tr>
    	<tr><td height="10%">项目文件:</td><td height="10%"><a href="${c.touziPoj.by1}">${c.touziPoj.by1}</a></td></tr>
