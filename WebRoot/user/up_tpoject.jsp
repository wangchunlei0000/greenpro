...
<script language="javascript">
  ...
  function   action1()      
  {      
        document.form1.action="touzipoj_upzancun.action";      
  ...
  <body>
	...
	%>
    <form name="form1" id="form1" action="touzipoj_update.action" enctype="multipart/form-data" method="post">
    <center>
    ...
    <table class="yy">
    	...
    	<s:if test="(#ty.id)==p.touziType.id"><input type="radio" name="r2.id" value="${ty.id}" checked="checked">${ty.style}&nbsp; &nbsp;   &nbsp;</s:if>
		<s:else><input type="radio" name="r2.id" value="${ty.id}">${ty.style}&nbsp; &nbsp;   &nbsp; 
		</s:else>
		</s:iterator>
    	</td>
    	</tr> 
    	<tr><td>目标金额</td><td><input type="text" name="p.money" value="<%=big %>" onkeyup="clearNoNum(this)"/>
    	<input type="hidden" name="p.pojid" value="${p.pojid}" />
    	<input type="hidden" name="p.requesttime" value="${p.requesttime}" />
    	<input type="hidden" name="u.userid" value="${session.userid}"/></td></tr>
    	<tr><td>所在地区</td><td><input type="text" name="p.area" value="${p.area}"/></td></tr>
    	<tr><td>投资期限</td><td><input type="text" name="p.period" value="${p.period}" onkeyup="clearNoNumm(this)"/></td></tr>
