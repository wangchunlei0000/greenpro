...
div.container {
  ...
  <body>
     ...
     <form action="touzipoj_adminfindall.action" method="post">
    <table>
    ...
    <a href="touzipoj_adminfindall.action?r1.id=&r2.id=&p.name=" class="zhubu">查看全部</a>
    ...
    <s:iterator value="poj" var="lpp">
    	...
    	<center>
    	<a href="cytpoj_users?po.pojid=${lpp.pojid}">${lpp.name}</a>&nbsp;&nbsp;
    		<s:if test="(#lpp.touziState.id)==1">
    		<a href="touzipoj_tong.action?p.pojid=${lpp.pojid}">通过</a>&nbsp;&nbsp;
				<a href="touzipoj_jujue.action?p.pojid=${lpp.pojid}">拒绝</a>
				
			</s:if>
		<s:elseif test="(#lpp.touziState.id)==8">
	 &nbsp;&nbsp;<a href="touzipoj_tong.action?p.pojid=${lpp.pojid}">去除举报</a>
		&nbsp;&nbsp;<a href="touzipoj_delete.action?p.pojid=${lpp.pojid}" onclick="return confirm('确定删除?');">删除</a></s:elseif>
         </center>
    	</div>
    </s:iterator></div>
    <div  style="width:100%;float:left;"><center>
     <form method="post" action="touzipoj_test0.action">
       	

     
	    	<s:if test="pp.currentPage==1">上一页</s:if>
	    	<s:else><a href="touzipoj_test0.action?pp.currentPage=${pp.currentPage-1}&pp.pageCount=${pp.pageCount}">上一页</a></s:else>
	    	&nbsp;&nbsp;
	    	<s:if test="pp.currentPage==pp.pageCount">下一页</s:if>
	    	<s:else><a href="touzipoj_test0.action?pp.currentPage=${pp.currentPage+1}&pp.pageCount=${pp.pageCount}">下一页</a></s:else>
	    	&nbsp;&nbsp;
	    	共${pp.pageCount}页&nbsp;第&nbsp;
